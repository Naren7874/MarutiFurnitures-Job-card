...
import { LayoutDashboard, ClipboardList, LogOut, Factory, PanelLeft, Bell } from 'lucide-react';
...
const NAV = [
    { label: 'Dashboard', icon: LayoutDashboard, path: '/factory' },
    { label: 'Job Cards', icon: ClipboardList, path: '/factory/jobcards' },
    { label: 'Notifications', icon: Bell, path: '/factory/notifications' },
];

export default function FactoryManagerLayout() {
                ...
                <div className={cn('flex items-center gap-3 px-4 py-3.5 border-b border-border', collapsed && 'justify-center px-0')}>
                    ...
                    {!collapsed && (
                        <div className="overflow-hidden">
                            ...
                            <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest opacity-60 truncate">Production Portal</p>
                        </div>
                    )}
                </div>
                    ...
                    {!collapsed && (
                        <div className="flex items-center gap-2 px-1">
                            ...
                            <div className="overflow-hidden flex-1">
                                ...
                                <p className="text-muted-foreground text-[10px] truncate">Factory Manager</p>
                            </div>
                        </div>
                    )}
                    <button
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <header className="flex items-center justify-between px-4 py-3 border-b border-border bg-background/80 backdrop-blur-md sticky top-0 z-40">
                    <button
                        ...
                        className="size-10 flex items-center justify-center rounded-xl bg-muted/40 border border-border text-muted-foreground hover:text-foreground hover:bg-muted hover:border-orange-500/30 transition-all duration-300"
                    >
                        <PanelLeft size={18} className={cn('transition-all duration-500', !collapsed ? 'text-orange-500' : 'opacity-70')} />
                    </button>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            <Bell size={18} />
                            {unreadCount > 0 && (
                                <span className="absolute top-1.5 right-1.5 w-4 h-4 bg-orange-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center shadow-[0_0_8px_rgba(249,115,22,0.4)]">
