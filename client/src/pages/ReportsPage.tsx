    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center justify-between">
                <p className="text-[11px] font-black uppercase tracking-[0.15em] text-muted-foreground/50">{label}</p>
                <div className={cn('p-2 rounded-xl', color)}><Icon size={14} /></div>
            </div>
            <p className="text-2xl font-black text-foreground">{value}</p>
            {change && <p className="text-[11px] font-bold text-emerald-500">{change}</p>}
        </motion.div>
    );
}
        ...
        <div className="p-6 md:p-8 max-w-[1600px] mx-auto space-y-10">
            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground/60 text-[13px] font-bold mt-2 uppercase tracking-wide">Business overview and performance metrics</p>
                </div>
                <div className="flex gap-3">
                    <Button variant="outline" className="rounded-xl h-10 text-[11px] font-black uppercase tracking-widest gap-2.5 border-border/60">
                        <Calendar size={14} /> This Month
                    </Button>
                    {canExport && (
                        <Button variant="outline" className="rounded-xl h-10 text-[11px] font-black uppercase tracking-widest gap-2.5 border-border/60" onClick={handleExportCSV}>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                    ...
                    {!canViewFinancial && (
                        <div className="flex flex-col items-center justify-center py-10 opacity-30">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest italic text-center">Financial Data Restricted</p>
                        </div>
                    )}
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
                    ...
                    {canViewProduction ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-10 opacity-30">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest italic text-center">Production Data Restricted</p>
                        </div>
                    )}
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                    ...
                    {canViewFinancial ? (
                        <>
                            {[
                                ...
                                <div key={row.label} className="space-y-1">
                                    <div className="flex justify-between text-[11px]">
                                        <span className="font-bold text-foreground/70 uppercase tracking-widest">{row.label}</span>
                        ...
                        <div className="flex flex-col items-center justify-center py-10 opacity-30">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest italic text-center">Quotation Data Restricted</p>
                        </div>
                    )}
                </motion.div>
